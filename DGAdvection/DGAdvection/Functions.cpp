...
VectorXd getFlux(ArrayOfElements v, int index, double a, double alpha, double time, bool isPeriodic)
{
	...
	if (isPeriodic)
	{
		returnVec(0) = a * v(index).u(N) - (0.5*(a*v(index).u(N) + a * v(v(index).getNext()).u(0))) + a * (1 - alpha) / 2.0 * (v(index).u(N)*v(index).getRightNormal() + v(v(index).getNext()).u(0)*v(v(index).getNext()).getLeftNormal());
		returnVec(N) = -a * v(index).u(0) + (0.5*(a*v(index).u(0) + a * v(v(index).getPrev()).u(N))) + a * (1 - alpha) / 2.0 * (v(v(index).getPrev()).u(N)*v(v(index).getPrev()).getRightNormal() + v(index).u(0)*v(index).getLeftNormal());
	}
	else if (!isPeriodic)
	{
		...
		{
			double inflow = 5.0* exp(-5.0*pow(-time / 2.0 - 5, 2));
			returnVec(N) = a * v(index).u(N) - (0.5*(a*v(index).u(N) + a * v(v(index).getNext()).u(0))) + a * (1 - alpha) / 2.0 * (v(index).u(N)*v(index).getRightNormal() + v(v(index).getNext()).u(0)*v(v(index).getNext()).getLeftNormal());
...
ArrayOfElements RHS(ArrayOfElements grid, double time, double a, double alpha, bool isPeriodic)
{
	...
	for (int i = 0; i < grid.size(); i++) //loops through each element
	{
		...
		updatedU = grid(i).VanderMatrix*flux - a * grid(i).metric* grid(i).DiffMatrix *grid(i).u;
